...
export async function build(model, projectData, projectDir, renderOpts = {}) {
  ...
  if (videoClips.length === 0) {
    ...
    for (let i = 0; i < videoClips.length; i++) {
      ...
      // FFCreator handles only built-in (non-gl:) transitions.
      // gl: transitions are intentionally skipped here — they are handled by the
      // FFmpeg pipeline (ffmpeg-renderer.js) which calls resolveTransitionFilter.
      let transEffect = null;
      if (transition && !transition.effect.startsWith('gl:')) {
        transEffect = TRANSITION_MAP[transition.effect] ?? DEFAULT_TRANSITION;
      }
